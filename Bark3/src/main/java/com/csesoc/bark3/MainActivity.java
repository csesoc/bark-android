...
public class MainActivity extends AppCompatActivity {
        ...
        tokenList.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (item != null) {
                    ...
                    toScan.putExtra(ScanActivity.INTENT_TOKEN, item.token);
                    toScan.putExtra(ScanActivity.INTENT_NAME, item.name);
                    toScan.putExtra(ScanActivity.INTENT_LOCATION, item.location);
                    toScan.putExtra(ScanActivity.INTENT_START_TIME, item.startTime);
                    toScan.putExtra(ScanActivity.INTENT_END_TIME, item.endTime);
                    toScan.putExtra(ScanActivity.INTENT_RUNNING, item.running);
    ...
    private class CheckToken extends AsyncTask<String, Void, JSONObject> {
        ...
        protected void onPostExecute(JSONObject result) {
            ...
            if (success) { // go to scan activity on success, passing on the event info

                try {
                    name = result.getString(ScanActivity.INTENT_NAME);
                    location = result.getString(ScanActivity.INTENT_LOCATION);
                    startTime = result.getInt(ScanActivity.INTENT_START_TIME);
                    endTime = result.getInt(ScanActivity.INTENT_END_TIME);
                    running = result.getBoolean(ScanActivity.INTENT_RUNNING);
                } catch (JSONException e) {
                    e.printStackTrace();
                }
    ...
    private class Token {
        ...
        private Token(String name, String token, String location, int startTime, int endTime, boolean running) {
            this.name = name;
            this.token = token;
